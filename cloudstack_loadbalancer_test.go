...
func TestGetLoadBalancer(t *testing.T) {
	...
	// reconciled towards is the preferred address, not the order CloudStack listed them in.
...
func TestEnsureLoadBalancer(t *testing.T) {
	...
	t.Run("rule named by the old provider is adopted and renamed", func(t *testing.T) {
		// Migration from the in-tree provider, whose rule names carried no protocol. The rule
		// must be adopted rather than recreated, so the README can stop asking operators to
		// delete their rules before migrating.
		ctrl := gomock.NewController(t)
		t.Cleanup(ctrl.Finish)
...
func TestPruneRulesScopesNetworkACLsToTheirNetwork(t *testing.T) {
	...
	t.Run("a rule in a deleted network keeps its ACL rule", func(t *testing.T) {
		...
		lb, m := newLB(ctrl)
		m.network.EXPECT().GetNetworkByID("net-old", gomock.Any()).Return(nil, 0, nil)
