...
func TestListAllFailsOnAGrowingResultSetRunningAway(t *testing.T) {
	...
	// as it is consumed would never satisfy the loop condition. The page cap
	// stops the walk, and it has to fail rather than hand back what it holds:
	...
	got, err := listAll(server, server.list)
	if err == nil {
		t.Fatalf("collected %d records with no error, want a failure at the cap", len(got))
	}
	if got != nil {
		t.Errorf("items = %v, want nil so a partial list cannot be used", got)
	}
